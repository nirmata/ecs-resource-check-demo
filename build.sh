...
docker buildx build . -t ecs-lambda-function --no-cache=true --platform linux/amd64,linux/x86_64 --provenance=false -f Dockerfile.kyverno-json
...
#docker buildx . -t ecs-lambda-function --no-cache=true --platform=linux/arm64 -f Dockerfile.nctl
